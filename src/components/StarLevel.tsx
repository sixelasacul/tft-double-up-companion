...
function Star({ className, isEnabled, onClick }: StarProps) {
  return (
    <Star8
      data-state={isEnabled ? "on" : "off"}
      className={cn(
        "h-10 stroke-black stroke-8 text-main hover:text-star-muted data-[state=on]:text-star",
        className
      )}
      onClick={onClick}
    />
  ...
  return (
    <div className="flex flex-row">
      ...
      <Star
        ...
        className="peer/2 peer-hover/1:data-[state=on]:text-star-muted [&:has(~:hover)]:data-[state=off]:text-star-muted"
      />
      <Star
        isEnabled={starLevel >= 3}
        onClick={() => onUpdateStarLevel?.(3)}
        className="peer-hover/1:data-[state=on]:text-star-muted peer-hover/2:data-[state=on]:text-star-muted"
      />
    </div>
  );
}
